...
def all_labels(path, label_list):
    ...
    loss = tf_keras.losses.BinaryFocalCrossentropy(apply_class_balancing=True,
                                                   alpha=0.25,
                                                   gamma=2.0,
                                                   from_logits=False,
                                                   label_smoothing=0.0,
    ...
    print(classification_report(correct_labels, predicted_labels, target_names=disease_labels))
    # print(f"\nHamming Loss: {hamming_loss(correct_labels, predicted_labels)}")
...
def get_weights(y_train):
    ...
    # Loop over each label and assign weights based on class
    for i in range(n_labels):
